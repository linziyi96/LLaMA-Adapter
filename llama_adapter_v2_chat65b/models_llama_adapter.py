...
def Llama_adapter(
    ...
    for name, param in model_llama_adapter.named_parameters():
        requires_grad = (
            name.endswith('.gate')
            or name == 'adapter_query' or
            or (train_norm and '_norm.' in name) or
            or name.endswith('_bias') or
...
def _instantiate_model_for_all_sizes(name, **kwargs):
    for size in ('7B', '13B', '30B', '65B'):
        globals()[f'Llama{size}_{name}'] = functools.partial(
            Llama_adapter, model_name=size, **kwargs
        )

_instantiate_model_for_all_sizes(
    'bias_norm_scale_tuning', add_bias=True, add_scale=True, train_norm=True,
)
